...
class OmniGenProcessor:
    ...
    def process_multi_modal_prompt(self, text, input_images):
        ...
        assert unique_image_ids == list(range(1, len(unique_image_ids)+1)), f"image_ids must start from 1 and be continuous, e.g. [1, 2, 3], got {unique_image_ids}"
        assert len(unique_image_ids) == len(input_images), f"total images must match number of image tags, got {len(unique_image_ids)} tags and {len(input_images)} images"
        ...
        for i in range(len(prompt_chunks)):
            ...
            if i != len(prompt_chunks) - 1:
                ...
                size = input_images[i].shape[-2] * input_images[i].shape[-1] // 16 // 16  # Assumes MindSpore tensor shape
    ...
    def __call__(self, 
        ...
        for i in range(len(instructions)):
            ...
            if use_input_image_size_as_output:
                input_data.append((mllm_input, neg_mllm_input, img_cfg_mllm_input, [mllm_input['pixel_values'][0].shape[-2], mllm_input['pixel_values'][0].shape[-1]]))
            else:
...
class OmniGenCollator:
    ...
    def create_mask(self, attention_mask, num_tokens_for_output_images):
        ...
        for mask in attention_mask:
            ...
            image_mask = ops.zeros((temp_l+1, img_length), ms.float32)
            temp_mask = ops.concat([temp_mask, image_mask], axis=-1)

            image_mask = ops.ones((img_length, temp_l+img_length+1), ms.float32)
            temp_mask = ops.concat([temp_mask, image_mask], axis=0)

            if pad_l > 0:
                pad_mask = ops.zeros((temp_l+1+img_length, pad_l), ms.float32)
                temp_mask = ops.concat([pad_mask, temp_mask], axis=-1)

                pad_mask = ops.ones((pad_l, seq_len), ms.float32)
            ...
            if pad_img_length > 0:
                ...
                temp_padding_imgs = ops.zeros((1, pad_img_length, self.hidden_size), ms.float32)
            else:
