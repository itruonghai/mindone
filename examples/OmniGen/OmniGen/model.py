...
from mindspore.common.initializer import TruncatedNormal, Zero
...
class TimestepEmbedder(nn.Cell):
    ...
    def timestep_embedding(t, dim, max_period=10000):
        ...
        half = dim // 2
        freqs = ops.exp(
            -math.log(max_period) * ms.numpy.arange(0, half, dtype=ms.float32) / half
        )
        args = ops.expand_dims(t, 1).astype(ms.float32) * freqs[None]
...
def get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0, 
                           interpolation_scale=1.0, base_size=1):
...
class PatchEmbedMR(nn.Cell):
    ...
    def construct(self, x):
        x = self.proj(x)
        x = ops.reshape(x, (x.shape[0], x.shape[1], -1))  # NCHW -> NLC
        x = ops.transpose(x, (0, 2, 1))
        return x

class OmniGen(nn.Cell, PreTrainedModel):
    """
    ...
    def __init__(
        ...
        pos_embed = get_2d_sincos_pos_embed(hidden_size, pos_embed_max_size, 
                                          interpolation_scale=pe_interpolation, 
                                          base_size=64)
        self.pos_embed = ms.Parameter(ms.Tensor(pos_embed[None, ...], ms.float32),
                                    requires_grad=False)
    ...
    #     ckpt = ms.load_checkpoint(os.path.join(model_name, 'model.ckpt'))
        
    #     ms.load_param_into_net(model, ckpt)
    #     return model
    ...
    def initialize_weights(self):
        ...
        def _basic_init(module):
            if isinstance(module, nn.Dense):
                module.weight.set_data(TruncatedNormal(0.02)(module.weight.shape))
                if module.bias is not None:
                    module.bias.set_data(Zero()(module.bias.shape))
                    
        self.apply(_basic_init)
        
        # Initialize patch_embed like nn.Linear
        w = self.x_embedder.proj.weight
        w.set_data(TruncatedNormal(0.02)(w.shape))
        self.x_embedder.proj.bias.set_data(Zero()(self.x_embedder.proj.bias.shape))

        w = self.input_x_embedder.proj.weight
        w.set_data(TruncatedNormal(0.02)(w.shape))
        self.input_x_embedder.proj.bias.set_data(Zero()(self.input_x_embedder.proj.bias.shape))

        # Initialize timestep embedding MLP
        self.t_embedder.mlp[0].weight.set_data(TruncatedNormal(0.02)(self.t_embedder.mlp[0].weight.shape))
        self.t_embedder.mlp[2].weight.set_data(TruncatedNormal(0.02)(self.t_embedder.mlp[2].weight.shape))
        self.time_token.mlp[0].weight.set_data(TruncatedNormal(0.02)(self.time_token.mlp[0].weight.shape))
        self.time_token.mlp[2].weight.set_data(TruncatedNormal(0.02)(self.time_token.mlp[2].weight.shape))

        # Zero-out output layers
        self.final_layer.adaLN_modulation[-1].weight.set_data(Zero()(self.final_layer.adaLN_modulation[-1].weight.shape))
        self.final_layer.adaLN_modulation[-1].bias.set_data(Zero()(self.final_layer.adaLN_modulation[-1].bias.shape))
        self.final_layer.linear.weight.set_data(Zero()(self.final_layer.linear.weight.shape))
        self.final_layer.linear.bias.set_data(Zero()(self.final_layer.linear.bias.shape))
    ...
    def unpatchify(self, x, h, w):
        ...
        """
        c = self.out_channels
        x = ops.reshape(x, (x.shape[0], h//self.patch_size, w//self.patch_size, 
    ...
    def cropped_pos_embed(self, height, width):
        ...
        spatial_pos_embed = ops.reshape(self.pos_embed, 
    ...
    def forward_with_cfg(self, x, timestep, input_ids, input_img_latents, input_image_sizes, 
        ...
        if use_img_cfg:
            ...
            cond, uncond, img_cond = ops.split(model_out, axis=0, output_num=3)
            cond = uncond + img_cfg_scale * (img_cond - uncond) + cfg_scale * (cond - img_cond)
            model_out = [cond, cond, cond]
        else:
            # Split for conditional and unconditional
            cond, uncond = ops.split(model_out, axis=0, output_num=2)
            cond = uncond + cfg_scale * (cond - uncond)
            model_out = [cond, cond]

        return ops.concat(model_out, axis=0), past_key_values

    def forward_with_separate_cfg(self, x, timestep, input_ids, input_img_latents, input_image_sizes, 
                                attention_mask, position_ids, cfg_scale, use_img_cfg, img_cfg_scale, 
                                past_key_values, use_kv_cache):
        ...
        for i in range(len(input_ids)):
            temp_out, temp_past_key_values = self.construct(
                x[i], timestep[i], input_ids[i], input_img_latents[i],
